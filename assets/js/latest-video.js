(()=>{
  ...
  fetch('/api/latest-video?playlist='+playlist,{signal:AbortSignal.timeout(9000),credentials:'omit'})
    ...
    .then(video=>{
      if(video.stale||video.unavailable||video.playlistId!==playlist||!/^[-\w]{11}$/.test(video.id||'')||typeof video.title!=='string')return;
      ...
      card.querySelector('[data-video-label]').textContent='Latest General Knowledge quiz';
